...
- **`main` ブランチ**
  安定動作している機能群が統合されている基本ブランチです。オールのZ座標（水深）基準のストローク自動検出ロジックや、それを可視化する [StrokeMetricsTable.tsx](file:///home/koki/BOVisualizer/src/components/StrokeMetricsTable.tsx) の追加、時系列グラフ上での位相帯の表示機能などが含まれています。
- **`test_newfunc` ブランチ (現在のブランチ)**
  `main` ブランチより1コミット先行しています。CSV内の非連続なインデックス値と再生フレーム位置の同期不整合を解消するリファクタリング、および [RowingMap.tsx](file:///home/koki/BOVisualizer/src/components/RowingMap.tsx) で毎フレーム地図がセンタリングされて動きが見えなくなる問題を解消する全体フィット処理 (`FitTrajectory`) などの改善が含まれています。
...
- **3D可視化エンジン** ([Scene.tsx](file:///home/koki/BOVisualizer/src/components/Scene.tsx))
  Three.jsを用いて艇体および左右オールのリアルタイム姿勢描画を行う。
- **GPSマッピング** ([RowingMap.tsx](file:///home/koki/BOVisualizer/src/components/RowingMap.tsx))
  Leafletを用いて地図上に航行軌跡を描画。
- **時系列折れ線グラフ** ([TimeSeriesChart.tsx](file:///home/koki/BOVisualizer/src/components/TimeSeriesChart.tsx))
  Rechartsを用いて、速度、加速度、角速度、SPM、SPLIT等のデータを同期プロット。
- **オール軌跡グラフ** ([OarTrajectoryChart.tsx](file:///home/koki/BOVisualizer/src/components/OarTrajectoryChart.tsx))
  ブレードの軌跡を2D/3Dのグラフとして描画。
- **再生制御と状態管理** ([PlaybackControls.tsx](file:///home/koki/BOVisualizer/src/components/PlaybackControls.tsx) / [playbackStore.ts](file:///home/koki/BOVisualizer/src/store/playbackStore.ts))
...
- **軌跡Z軸（水深）基準のストローク自動検出** ([strokeDetect.ts](file:///home/koki/BOVisualizer/src/utils/strokeDetect.ts))
...
- **時系列グラフ上のフェーズ背景色分け** (`6bdbb2e`)
  - [TimeSeriesChart.tsx](file:///home/koki/BOVisualizer/src/components/TimeSeriesChart.tsx) に、Rechartsの `ReferenceArea` を用いて、各フェーズに対応した薄い背景色（青・緑・橙・灰）を透過描画。
- **フェーズ単位のシークナビゲーション**
  - キーボードショートカット `Shift + ←` / `Shift + →` で、前後のフェーズ開始位置へフレームをスナップシークできる機能を追加。
- **ストローク毎の分析テーブル** ([StrokeMetricsTable.tsx](file:///home/koki/BOVisualizer/src/components/StrokeMetricsTable.tsx))
...
- **フレームインデックス同期の正常化** ([metrics.ts](file:///home/koki/BOVisualizer/src/utils/metrics.ts))
  - CSVの 'number' 列（非ゼロ開始や飛びがある実測値）を基準にGPSポイントのインデックスを生成していたため、再生位置インデックスとズレていた問題を修正。フレーム配列の配列添字 (index) をキーに同期するようにした。
- **地図のフィット挙動の最適化** ([RowingMap.tsx](file:///home/koki/BOVisualizer/src/components/RowingMap.tsx))
...
- **リセンターボタンの挙動改善** ([RowingMap.tsx](file:///home/koki/BOVisualizer/src/components/RowingMap.tsx))
