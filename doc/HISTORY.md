...
### Phase 8: システム全体の再構築と境界の型安全化 (2026-08-13)

この更新はGitHub Pagesで配信する静的SPAとローカル検証の範囲を対象とする。production用のcloud runtimeやbackendは導入しない。

- **データ境界の再構築**
  - CSVパーサーを `src/data` へ移し、リモート取得とローカルファイル入力を同一の `DatasetCsv` 契約へ統一。
  - ローカルフォルダ内の不正なCSVを個別に隔離し、正常なファイルの読み込みを継続できるようにした。
- **状態・非同期処理の型安全化**
  - Zustandの全スライスを `PlaybackStoreState` で型付けし、ストア境界の `any` を除去。
  - データセット読み込み、横断解析、ローカル自動更新で古い非同期応答が最新状態を上書きしない構成へ変更。
- **解析と表示の分離**
  - ストローク別メトリクスを `strokeMetricsAnalyzer` として解析キャッシュへ統合し、表と軌跡チャート内の重複計算を廃止。
  - `AnalysisResultMap` と `getAnalysisResult()` により追加アナライザー結果を型付きで取得可能にした。
- **再生・描画基盤の共通化**
  - 長時間停止後にも正しく追従する再生クロックへ刷新し、キーボード操作を専用フックへ分離。
  - 2つのCanvasグラフでサイズ、DPR、初期レイアウト待機、再描画処理を共通化。
- **依存と検証の整理**
  - 未使用となったRecharts、PapaParseおよび関連型依存を削除。
  - lint、production build、65件の単体テスト、および複数ビューポートのCDP E2Eで同期表示を検証。

---

## 3. 全体構造見直しに向けた最適化の検討事項

本章はPhase 7着手時点の監査記録です。Phase 1〜6 で「つぎ足し」により積み上がった当時のコードを棚卸しし、(A) 処理コストの削減（最適化）と (B) 新機能追加に耐える拡張性の確保、の2軸でリファクタリング方針を整理しています。現在の構成はPhase 8および [DETAIL.md](./DETAIL.md) を参照してください。
