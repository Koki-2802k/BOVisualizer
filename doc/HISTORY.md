...
- **`main` ブランチ**
  安定動作している機能群が統合されている基本ブランチです。オールのZ座標（水深）基準のストローク自動検出ロジックや、それを可視化する [StrokeMetricsTable.tsx](../src/components/StrokeMetricsTable.tsx) の追加、時系列グラフ上での位相帯の表示機能などが含まれています。
- **`test_newfunc` ブランチ (現在のブランチ)**
  `main` ブランチより1コミット先行しています。CSV内の非連続なインデックス値と再生フレーム位置の同期不整合を解消するリファクタリング、および [RowingMap.tsx](../src/components/RowingMap.tsx) で毎フレーム地図がセンタリングされて動きが見えなくなる問題を解消する全体フィット処理 (`FitTrajectory`) などの改善が含まれています。
